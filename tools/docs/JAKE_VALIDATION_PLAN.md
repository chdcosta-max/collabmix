...
| Sync-state reliability (natural-BPM sticky session tempo + clean reset + de-churn) | `?synctempo=` (planned) | **NOT BUILT** (diagnosed; design pending empirical confirm) | Load 4+ tracks in sequence, toggle sync on/off, play each | Sync does NOT degrade/fail by the 4th; new tracks adopt session tempo (option i); full release resets to a clean slate |
