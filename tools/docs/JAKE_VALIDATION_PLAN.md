...
| Sync-state reliability (natural-BPM sticky session tempo + master freeze + clean reset + de-churn) | `?synctempo=` (default ON, `=0` legacy) | SHIPPED default-ON, Desktop-validated — **awaiting Jake** | Load 4+ tracks, toggle sync on/off, play each; reassign master (M on B) | Sync does NOT degrade/fail by the 4th; master takes (LOCKED master=B); session period from natural (not contaminated); new tracks adopt it; full release resets clean. Desktop ✓ (rate=1.0000 across 4 cycles, B-master takes, clean reset). |
